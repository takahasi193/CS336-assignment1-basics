...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    config=Config(d_model=d_model,num_heads=num_heads,max_seq_len=max_seq_len,theta=theta)
    attn=model.CausalAttention(config,use_rope=True)
    state_dict={
        'q_proj.weight':q_proj_weight,
        'k_proj.weight':k_proj_weight,
        'v_proj.weight':v_proj_weight,
...
def run_transformer_block(
    ...
    """
    config=Config(d_model=d_model,num_heads=num_heads,d_ff=d_ff,max_seq_len=max_seq_len,theta=theta)
    block=model.TransformerBlock(config)
...
def run_transformer_lm(
    ...
    config=Config(vocab_size=vocab_size,
        ...
        d_ff=d_ff,
        theta=rope_theta)
    transformer=model.Transformer_LM(config)
...
def run_get_batch(
    ...
    """
    config=Config(batch_size=batch_size,max_seq_len=context_length,device=device)
    return get_batch(dataset,config)
